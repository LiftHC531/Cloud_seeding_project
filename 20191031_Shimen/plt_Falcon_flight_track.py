...
def read_Falcon_data(ff):
    ...
    #date_utc = data[col_name[12]].tolist() # Cas2019e0603 (LST)
    date_utc = data[col_name[13]].tolist() # Cas2019e0603 (LST)
    time = [str(dd)[8:14] for dd in date_utc]
...
(lon, lat, time) = read_Falcon_data(Files[2])
''' Deal with NaN '''
'''
...
# Plot flight track
# seeding time: 1318-1328 LST (0518-0528 UTC)
lnres = Ngl.Resources()
lnres.gsLineColor = 'white'
lnres.gsLineThicknessF = 10
for i in range(len(lon)):
    if np.int(time[i][0:4]) == 0:
       lon[i] = 0.
       lat[i] = 0.

for i in range(len(lon)-1):
    #print (time[i][0:4])
    #print (np.int(time[i][0:4]))

    xd = [lon[i], lon[i+1]]
    yd = [lat[i], lat[i+1]]
    if np.int(time[i][0:4]) < 510 and np.int(time[i][0:4]) > 0:
       #print(i)
       #print (np.int(time[i][0:4]))
       lnres.gsLineColor = 'white'
       #print(yd)
       ...
       plt_marker = add_polymarker(wks,plot,lon[i],lat[i], \
         index=1, size=0.005, color=lnres.gsLineColor)
    elif np.int(time[i][0:4]) >= 510 and np.int(time[i][0:4]) <= 517:
       lnres.gsLineColor = 'red'
       #plt_track = Ngl.add_polyline(wks, plot, xd, yd, lnres)
       plt_marker = add_polymarker(wks,plot,lon[i],lat[i], \
         index=1, size=0.005, color=lnres.gsLineColor)
    elif np.int(time[i][0:4]) > 517 and np.int(time[i][0:4]) < 529:
       ...
       plt_marker = add_polymarker(wks,plot,lon[i],lat[i], \
         index=1, size=0.005, color=lnres.gsLineColor)
    plt_marker = add_polymarker(wks,plot,lon[-1],lat[-1], \
      index=1, size=0.005, color=lnres.gsLineColor)
